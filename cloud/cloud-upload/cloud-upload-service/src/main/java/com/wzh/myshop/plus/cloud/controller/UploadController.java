...
import com.wzh.myshop.plus.cloud.dto.FileInfo;
...
public class UploadController {
    ...
    public ResponseResult<FileInfo> upload(MultipartFile multipartFile) {
        ...
        try {
            ...
            return new ResponseResult<FileInfo>(CodeMessage.LOGIN_SUCCESS.getCode(), "文件上传成功", new FileInfo("http://" + BUCKET_NAME + "." + ENDPOINT + "/" + newName));
        } catch (IOException e) {
            return new ResponseResult<FileInfo>(20004, "文件上传失败，请重试");
        } finally {
            client.shutdown();
        }
    }
